...
PixelShader = 
{
		...
		float vOutlineMult, float2 vOutlineCutoff, float2 vCamDistOverride, inout float vBloomAlpha )
	{
		...
		float3 vGradMix = float3(0.0f,0.0f,0.0f);
		float3 gradientColor = tex2D( TexCh1, vUV );
		
		float vAlpha1 = 0.0f;
		float vAlpha2 = 0.0f;
		
		if (gradientColor.x > 0.01f, gradientColor.y > 0.01f, gradientColor.z > 0.01f) {
			...
			vAlpha2 = gradient_border_process_channel( vGradMix, vColor, vGBCamDistCh2, vNormal, vUV2, TexCh1, TexCh2, vOutlineMult, vOutlineCutoff.y, (1.0 - vAlpha1 * GB_STRENGTH_CH1 * GB_FIRST_LAYER_PRIORITY) * GB_STRENGTH_CH2 );
