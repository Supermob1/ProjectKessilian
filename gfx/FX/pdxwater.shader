...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT_WATER Input ) : PDX_COLOR
		{
			...
			//if ((Input.pos.x > 2397 && Input.pos.x < 2431) && (Input.pos.z == 1455 || (Input.pos.z > 1455)))
			//	discard;
			//if ((Input.pos.x > 2397 && Input.pos.x < 2399))
			//	discard;
			
			float waterHeight = MultiSampleTexX( HeightTexture, Input.uv ) / ( 95.7f / 255.0f );
			if (waterHeight > 0.95f) {
				float HalfPix = 0.5f / GB_TextureHeight;
				float3 colorTest = tex2D( GradientBorderChannel1, float2(Input.uv.x, Input.uv.y*(0.5f - HalfPix)));
