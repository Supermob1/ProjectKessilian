...
PixelShader =
{
    ...
    [[
        #ifdef SPACE_MODE
            // Space mode: sample the cubemap as a static starfield
            float3 starColor = texCUBE( ReflectionCubeMap, Input.cubeRotation ).rgb;
            return float4( starColor, 1.0f );
        #else

        // --- Original water shader code begins ---
        float3 ApplyIce( float3 vColor, float2 vPos, inout float3 vNormal, float4 vMudSnowColor, float2 vIceUV, out float vIceFade )
        {
            ...
            vNormal = normalize( lerp( vNormal, normalize( vIceDiffuse.rgb - 0.5f ), vIceFade ) );
        ...
        float4 main( VS_OUTPUT_WATER Input ) : PDX_COLOR
        {
            ...
            B *= vFlatten; M *= vFlatten * vFlatten;
        #ifdef LOW_END_GFX
            ...
            lightingProperties._Glossiness = (spec/9.0f) * (1-vSpecMap);
        ...
        // --- Original water shader code ends ---
...
Effect water_space
{
    VertexShader = "VertexShader"
    PixelShader  = "PixelShader"
    Defines      = { "SPACE_MODE", "NO_WAVES" }
}

// Alias for backwards compatibility
Effect space_water = water_space
